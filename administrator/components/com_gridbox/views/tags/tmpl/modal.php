    ...
    document.addEventListener('DOMContentLoaded', function(){
        ...
        for (var i = 0; i < items.length; i++) {
            items[i].addEventListener('click', function(event){
                ...
                window.parent.document.querySelector('#jform_request_tag_name').value = this.dataset.title;
                window.parent.document.querySelector('#jform_request_tag_id').value = this.dataset.id;
                window.parent.jQuery('#gridbox-category-modal').modal('hide');
                var value = window.parent.document.querySelector('#jform_link').value.replace(/&tag=\d+/, '');
                window.parent.document.querySelector('#jform_link').value = value+'&tag='+this.dataset.id;
            });
        }
    });
</script>
<div class="modal-shortcode">
    <form action="<?php echo JRoute::_('index.php?option=com_gridbox&view=tags&layout=modal&tmpl=component&id='.$id); ?>"
